...
class VideoClsDataset(Dataset):
    ...
    def __init__(self,
        ...
        if args.use_text_template and mode == 'train':
            self.video_text_features = torch.load(args.video_text_template_path)
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            if args.video_num_sample > 1:
                frame_list = []
                label_list = []
                index_list = []
                for _ in range(args.video_num_sample):
            ...
            #     template_idx = random.randint(0,6)
            #     text_feature = self.video_text_features[self.label_array[index]][template_idx]
            #     return buffer, self.label_array[index], index, text_feature
            ...
            rtn={
                'video': buffer,
                'caption': self.test_label_array[index]
            }
    ...
    def _aug_frame(self, buffer, args):
        aug_transform = video_transforms.create_random_augment(
            ...
            auto_augment=args.video_aa,
            interpolation=args.video_train_interpolation,
        )
        ...
        buffer = spatial_sampling(
            ...
            random_horizontal_flip=False if args.video_dataset == 'SSV2' else True,
        ...
        if self.rand_erase:
            erase_transform = RandomErasing(
                args.video_reprob,
                mode=args.video_remode,
                max_count=args.video_recount,
                num_splits=args.video_recount,
                device="cpu",
            )
